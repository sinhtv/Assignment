...
class ControllerStep3 extends Controller {
	...
	public function index() {
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$file = fopen(DIR_OPENCART . 'admin/view/plugins/ckeditor/config.js', 'w');
			$str = file_get_contents(DIR_OPENCART . 'admin/view/plugins/ckeditor/config.js');
			$str = str_replace("{{HTTP_OPENCART}}", DIR_OPENCART. 'admin/view/',$str);
			fwrite($file, $str);
			fclose($file);

			$file = fopen(DIR_OPENCART . 'admin/view/plugins/ckfinder/config.php', 'w');
			$str = file_get_contents(DIR_OPENCART . 'admin/view/plugins/ckfinder/config.php');
			$str = str_replace("{{HTTP_OPENCART}}", DIR_OPENCART,$str);
			fwrite($file, $str);
			fclose($file);
