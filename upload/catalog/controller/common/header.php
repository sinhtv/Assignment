...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		$data['background'] = '';
		$modules = $this->model_design_layout->getLayoutModules($layout_id, 'header_background');
		foreach ($modules as $module) {
			...
			if (isset($part[1])) {
				...
				if ($setting_info && $setting_info['status']) {
					$banners = $this->model_design_banner->getBanner($setting_info['banner_id']);

					foreach ($banners as $banner) {
						if (is_file(DIR_IMAGE . $banner['image'])) {

							$data['background'] = $this->model_tool_image->resize($banner['image'], $setting_info['width'], $setting_info['height']);
							break 2;
						}
					}
				}
